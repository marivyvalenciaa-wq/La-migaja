...
app.post("/api/login", async (req, res) => {
  ...
  const username = String(usuario).trim().toLowerCase();
  const { data: row, error } = await supabase.from("usuarios").select("*").eq("username", username).maybeSingle();

  if (error) {
    console.error("ERROR DE SUPABASE:", error.message);
    return res.status(500).json({ error: "Error de conexión con la base de datos: " + error.message });
  }
